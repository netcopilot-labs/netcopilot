"""
XR SNMP Security Misconfiguration — Deep Python rule for the hybrid rule engine.

Detection Logic:
    Iterates over model devices, loads facts/config, checks for violations.

Rule ID: CIS_XR_1_5_SNMP
Severity: high
"""
...
class CisXrSnmpRule(BaseRule):
    """CIS IOS XR 1.5: SNMP communities must not use defaults, must have ACLs."""

    rule_id = "CIS_XR_1_5_SNMP"
    severity = "high"
    title = "XR SNMP Security Misconfiguration"
    description = "CIS IOS XR 1.5: SNMP communities must not use defaults, must have ACLs"
        ...
        for device in model.get("devices", []):
            ...
            if issues:
                findings.append(Finding.create_from_rule(
                    ...
                    recommendation="Remove default communities, use SNMPv3 with authentication and encryption",
                ))
